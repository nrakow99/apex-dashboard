  ...
  return (
    ...
    >
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            ...
            <Badge variant="outline" className={cn(
              ...
              account.firm === "Lucid" && "border-violet-500/50 text-violet-400",
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex justify-between text-[10px] mb-1.5">
            <span className="text-muted-foreground uppercase tracking-wider">
              {account.firm === "Lucid" ? "Max Loss Limit" : "Drawdown"}
            </span>
            <span className={cn(
              "font-mono font-medium",
              stats.drawdownRemaining > account.maxDrawdown * 0.5  ? "text-emerald-500" :
              stats.drawdownRemaining > account.maxDrawdown * 0.2  ? "text-amber-500"  : "text-red-500"
            )}>
          ...
          <Progress
            ...
            className={cn(
              ...
              stats.drawdownRemaining <= account.maxDrawdown * 0.5 &&
                stats.drawdownRemaining > account.maxDrawdown * 0.2 && "[&>div]:bg-amber-500",
        ...
        {account.type === "PA" && consistencyInfo && (
          <div className="flex items-center gap-4 pt-3 border-t border-border/50">
            <div className="flex items-center gap-2">
              <div className={cn("w-2.5 h-2.5 rounded-full", stats.tradingDays >= 5 ? "bg-emerald-500" : "bg-amber-500")} />
              <span className="text-sm text-muted-foreground">{stats.tradingDays}/5 Days</span>
            </div>
            <div className="flex items-center gap-2">
              <div className={cn("w-2.5 h-2.5 rounded-full", consistencyInfo.isValid ? "bg-emerald-500" : "bg-red-500")} />
        ...
        {/* Lucid PA: scaling indicator */}
        {account.firm === "Lucid" && account.type === "PA" && (
          <div className="flex items-center gap-2 pt-3 border-t border-border/50">
            <div className="w-2.5 h-2.5 rounded-full bg-violet-500" />
            <span className="text-sm text-muted-foreground">Scaling Plan Active</span>
          </div>
        )}
      </div>
    </Card>
