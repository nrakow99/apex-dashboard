...
export function AccountTrajectory({ account, data, stats }: AccountTrajectoryProps) {
  ...
  const chartData = useMemo<Point[]>(() => {
    ...
    return points
  }, [account, data, rules.maxDrawdown, startingBalance, stats.activeEodFloor, stats.minBalance, today])
  ...
  return (
    <Card className="rounded-[14px] border-[#262629] bg-[#101012] p-5 sm:p-6">
      ...
      <div className="h-[250px] w-full sm:h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 8, bottom: 2, left: 0 }}>
            <CartesianGrid vertical={false} stroke="#242427" strokeDasharray="2 5" />
            <XAxis dataKey="date" axisLine={false} tickLine={false} minTickGap={34} tick={{ fill: "#6E6E73", fontSize: 10 }} dy={8} />
            <YAxis domain={domain} axisLine={false} tickLine={false} width={58} tick={{ fill: "#6E6E73", fontSize: 10 }} tickFormatter={(value) => `$${(value / 1000).toFixed(1)}k`} />
            <Tooltip content={<TrajectoryTooltip floorLabel={getChartFloorLineLabel(account)} />} cursor={{ stroke: "#48484D", strokeDasharray: "3 4" }} />
            <Line type="monotone" dataKey="balance" stroke="#F5F5F7" strokeWidth={2.25} dot={false} activeDot={{ r: 3, fill: "#FFFFFF", stroke: "#000000", strokeWidth: 1 }} />
            <Line type="stepAfter" dataKey="floor" stroke="#6E6E73" strokeWidth={1.5} strokeDasharray="5 5" dot={false} activeDot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      <div className="mt-4 flex flex-wrap items-center gap-5 border-t border-[#262629] pt-4 text-[10px] text-[var(--muted)]">
        <span className="inline-flex items-center gap-2"><i className="h-px w-5 bg-white" />Balance</span>
        <span className="inline-flex items-center gap-2"><i className="w-5 border-t border-dashed border-[#6E6E73]" />{getChartFloorLineLabel(account)}</span>
      </div>
    </Card>
  )
}
...
function TrajectoryTooltip({ active, payload, floorLabel }: {
  ...
  return (
    <div className="min-w-[190px] rounded-[10px] border border-[#303034] bg-[#171719] p-3">
      ...
      <dl className="mt-3 space-y-2 text-[11px]">
        ...
        <div className="flex justify-between gap-5 border-t border-[#303034] pt-2"><dt className="text-[var(--muted)]">Daily P&L</dt><dd className={`font-mono ${pnlColorClass(point.dailyPnl)}`}>{formatPnL(point.dailyPnl)}</dd></div>
      </dl>
    </div>
  )
}
