...
import { exportPayoutsCsv, exportTradesCsv, exportWorkspaceJson } from "@/lib/data-export"
import type { Account, Payout, Trade } from "@/lib/types"

interface Props { accounts: Account[]; trades: Trade[]; payouts: Payout[] }
...
export function DataManagementPanel({ accounts, trades, payouts }: Props) {
    ...
    <section className="border border-[var(--hairline)] bg-[var(--surface)]">
      <div className="flex flex-col gap-4 border-b border-[var(--hairline)] px-5 py-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" onClick={() => downloadText(`propdash-workspace-${exportDate()}.json`, exportWorkspaceJson(accounts, trades, payouts, new Date().toISOString()), "application/json")}><FileJson />Workspace JSON</Button>
