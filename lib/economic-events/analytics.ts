...
export const RED_FOLDER_MACRO =
  /\b(cpi|ppi|pce|non[- ]farm|payrolls|\bnfp\b|fomc|fed rate|interest rate decision|gdp|retail sales|ism|pmi|unemployment|jobless)\b/i

export function isRedFolderEvent(ev: Pick<EconomicEvent, "currency" | "impact" | "title">): boolean {
  if (ev.currency !== "USD") return false
