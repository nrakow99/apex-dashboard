...
function parseEventInstant(row: ForexFactoryLikeRow, from: string): { instant: Date; hasSpecificTime: boolean } | null {
  const timestamp = readTimestamp(row)
  if (timestamp) return { instant: timestamp, hasSpecificTime: true }

  const datetime = readString(row, ["datetime", "dateTime", "date_time", "iso", "utc"])
  if (datetime) {
    if (/^\d{4}-\d{2}-\d{2}$/.test(datetime)) {
      const d = toDate(`${datetime} 12:00:00`, { timeZone: NY })
      return Number.isNaN(d.getTime()) ? null : { instant: d, hasSpecificTime: false }
    }

    if (/[zZ]$|[+-]\d{2}:?\d{2}$/.test(datetime)) {
      const d = new Date(datetime)
      return Number.isNaN(d.getTime()) ? null : { instant: d, hasSpecificTime: true }
    }

    const d = toDate(datetime.replace("T", " "), { timeZone: NY })
    if (!Number.isNaN(d.getTime())) return { instant: d, hasSpecificTime: true }
  }

  const date = readString(row, ["date", "day", "calendarDate", "eventDate"]) ?? from
  const time = readString(row, ["time", "eventTime", "releaseTime"])
  const hasSpecificTime = Boolean(time && !/tentative|all\s*day|day\s*\d+/i.test(time))
  const wallClock = hasSpecificTime ? `${date} ${time}` : `${date} 12:00:00`
  const d = toDate(wallClock, { timeZone: NY })

  return Number.isNaN(d.getTime()) ? null : { instant: d, hasSpecificTime }
}
...
function rowsFromResponse(json: unknown): ForexFactoryLikeRow[] {
  ...
  for (const candidate of candidates) {
    ...
    if (nested) {
      for (const key of ["events", "calendar", "economicCalendar", "economic_calendar"]) {
...
function normalizeRow(row: ForexFactoryLikeRow, index: number, from: string): EconomicEvent | null {
  ...
  const currency = readString(row, ["currency", "ccy", "symbol"])?.toUpperCase() ?? null
  const country = readString(row, ["country", "countryCode", "country_code"]) ?? (currency === "USD" ? "US" : currency ?? "ZZ")
  const parsed = parseEventInstant(row, from)
  if (!parsed) return null

  const { instant, hasSpecificTime } = parsed
